...
const CredentialDto = z.object({ username: z.string(), password: z.string() })
...
export const POST = async function(request: NextRequest) {
    ...
    return Response.json({ username: data.username, token: await generateToken({ _userId: newUser._id, role: 'user' }) });
}